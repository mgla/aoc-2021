        ...
        if chart_num in charts:
            for col_num in range(len(charts[chart_num]['cols'])):
                ...
                if len(charts[chart_num]['rows'][col_num]) == 0 or len(charts[chart_num]['cols'][col_num]) == 0:
                    ...
                    # calculate score
                    if len(charts) > 1:
                        del charts[chart_num]
                    else:
                        print(f' => last winning board is board #{chart_num} on roll {roll}')
                        result = 0
                        for row in charts[chart_num]['rows']:
                            result += sum([n for n in charts[chart_num]['rows'][row].keys()])
                        result *= roll
                        print(f"result = {result}")
                        aocd.submit(result, part="b", year=2021, day=4)
                    break  # num only once per chart
